...
class HomeOtherUpload extends StatefulWidget {
  final String tag;
  final String savePath;
  final Function onClose;

  const HomeOtherUpload(
      {required this.tag,
  ...
  @override
  State<HomeOtherUpload> createState() => HomeOtherUploadState();
}

class HomeOtherUploadState extends State<HomeOtherUpload> {
  List<OtherUploadDataType> selectList = [];
  bool isUpload = false;

  void selectFiles() {
    if (selectList.length >= uploadLimit) {
      CommonToast.showToast('选择的文件不能超过$uploadLimit个！');
      return;
    }
    final typeGroup =
        XTypeGroup(label: '文件', extensions: ['*.*']);
    openFiles(acceptedTypeGroups: [typeGroup]).then((value) {
      List<OtherUploadDataType> list = [];
  ...
  void uploadFiles() {
    ...
    for (var element in selectList) {
      ...
      Future.delayed(const Duration(milliseconds: loadTime), () {
        CommonUtil.uploadOtherFile(widget.savePath, element.filePath,
  ...
  Widget buildUploadState(context, OtherUploadDataType item) {
    if (item.showLoading) {
      ...
      if (item.uploadState == FileDataUploadState.uploadedFail) {
        return Text(
          item.uploadMsg,
          style: otherUploadStateMsgStyle,
        );
      } else {
        return Container();
      }
