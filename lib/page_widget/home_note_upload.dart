...
class HomeNoteUpload extends StatefulWidget {
  final String tag;
  final String savePath;
  final Function onClose;

  const HomeNoteUpload(
      {required this.tag,
  ...
  @override
  State<HomeNoteUpload> createState() => HomeNoteUploadState();
}

class HomeNoteUploadState extends State<HomeNoteUpload> {
  List<NoteUploadDataType> selectList = [];
  bool isUpload = false;

  void selectFiles() {
    if (selectList.length >= uploadLimit) {
      CommonToast.showToast('选择的笔记不能超过$uploadLimit个！');
      return;
    }
    final typeGroup =
        XTypeGroup(label: '笔记', extensions: CommonUtil.noteExtensionList());
    openFiles(acceptedTypeGroups: [typeGroup]).then((value) {
      List<NoteUploadDataType> list = [];
  ...
  void uploadFiles() {
    ...
    for (var element in selectList) {
      ...
      Future.delayed(const Duration(milliseconds: loadTime), () {
        CommonUtil.uploadNoteFile(widget.savePath, element.filePath,
  ...
  Widget buildUploadState(context, NoteUploadDataType item) {
    if (item.showLoading) {
