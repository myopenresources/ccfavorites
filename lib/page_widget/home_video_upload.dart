...
class HomeVideoUpload extends StatefulWidget {
  final String tag;
  ...
  @override
  State<HomeVideoUpload> createState() => HomeVideoUploadState();
}

class HomeVideoUploadState extends State<HomeVideoUpload> {
  List<VideoUploadDataType> selectList = [];
  bool isUpload = false;

  void selectFiles() {
    if (selectList.length >= uploadLimit) {
      CommonToast.showToast('选择的视频不能超过$uploadLimit个！');
      return;
    }
    final typeGroup =
        XTypeGroup(label: '视频', extensions: CommonUtil.videoExtensionList());
    openFiles(acceptedTypeGroups: [typeGroup]).then((value) {
      List<VideoUploadDataType> list = [];
  ...
  void uploadFiles() {
    ...
    for (var element in selectList) {
      ...
      Future.delayed(const Duration(milliseconds: loadTime), () {
        CommonUtil.uploadVideoFile(widget.savePath, element.filePath,
  ...
  Widget buildUploadState(context, VideoUploadDataType item) {
    if (item.showLoading) {
      ...
      if (item.uploadState == FileDataUploadState.uploadedFail) {
        return Text(
          item.uploadMsg,
          style: videoUploadStateMsgStyle,
        );
      } else {
        return Container();
      }
